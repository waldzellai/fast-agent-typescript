...
const routerLlm = { 
  send: jest.fn().mockImplementation((message: string) => {
    if (message.toLowerCase().includes('sunny')) {
      return Promise.resolve('I will route to the sunny agent\n\n***ROUTE sunny');
    } else {
      return Promise.resolve('I will route to the stormy agent\n\n***ROUTE stormy');
    }
  }),
  ...
  getModelFactory: jest.fn().mockImplementation((_context, modelName) => {
    ...
    return () => {
      ...
      if (modelName === 'passthrough' && _context?.agentName === 'sunny') {
        return sunnyLlm;
      } else if (modelName === 'passthrough' && _context?.agentName === 'stormy') {
        return stormyLlm;
  ...
  testModels.forEach(modelName => {
    it(`should route based on text content using ${modelName}`, async () => {
      ...
      fast.router({ 
        ...
        const sunnyResponse = await agent.weather.send('the weather is sunny');
        expect(sunnyResponse.toLowerCase()).toContain('beachball');

        // Test routing to stormy agent
        const stormyResponse = await agent.weather.send('storm clouds coming, looks snowy');
    ...
    it(`should route based on image content using ${modelName}`, async () => {
      ...
      fast.router({ 
        ...
        const sunnyResponse = await agent.weather.generate([
          Prompt.user(path.resolve(__dirname, 'sunny.png'), "here's the image")
        ]);
        expect(sunnyResponse.first_text().toLowerCase()).toContain('beachball');

        // Test routing with umbrella image
        const stormyResponse = await agent.weather.generate([
          Prompt.user(path.resolve(__dirname, 'umbrella.png'), "here's the image")
        ]);
        expect(stormyResponse.first_text().toLowerCase()).toContain('umbrella');
      });
