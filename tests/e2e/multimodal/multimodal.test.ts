...
// Extend ContentPart to include the tool types for testing purposes
...
mockSend = jest.fn() 
    .mockImplementation(async (message: string | PromptMessage | PromptMessageMultipart): Promise<string> => {
        let userContent = "";
        ...
        console.log(`mockSend received content: ${userContent.substring(0, 50)}...`);
...
mockGenerate = jest.fn() 
 .mockImplementation(async (messages: PromptMessage[]): Promise<PromptMessage | PromptMessageMultipart> => {
    ...
    if (userPromptMessage && userPromptMessage.content) {
        const userContentParts = typeof userPromptMessage.content === 'string' 
        ...
        console.log(`mockGenerate received user text: ${textPart?.text.substring(0, 50)}...`); 
        if (imagePart) console.log(`mockGenerate received image part.`);


        if (textPart?.text.includes("image fetch tool")) {
             responseContent.push({ type: 'tool_call', toolName: "image_fetch", input: { query: "sample image" }, callId: "call_123" } as ToolCallContent);
             const mockImage: ImageContent = { 
                 type: 'image', 
                 data: fs.readFileSync(imagePath).toString('base64'), 
                 mimeType: 'image/png' 
             };
             responseContent.push({ type: 'tool_result', callId: "call_123", output: "Mock tool text result about evalstate", resources: [mockImage] } as ToolResultContent);
             assistantResponseText = "Okay, I used the tool. The user name is evalstate.";
             responseContent.push({ type: 'text', text: assistantResponseText } as TextContent);

        } else if (textPart?.text.includes("summarise the sample PDF")) { 
            responseContent.push({ type: 'tool_call', toolName: "pdf_summarize", input: { query: "sample PDF" }, callId: "call_456" } as ToolCallContent);
            responseContent.push({ type: 'tool_result', callId: "call_456", output: "Mock PDF summary mentioning Fast-Agent" } as ToolResultContent);
            assistantResponseText = "I have summarized the PDF using the tool. It mentions Fast-Agent.";
             responseContent.push({ type: 'text', text: assistantResponseText } as TextContent);
        } else if (imagePart) { 
    ...
    const assistantResponse = Prompt.message({role: Role.Assistant}, ...responseContent);

    (assistantResponse as any).all_text = () => responseContent.filter(isTextContent).map(p => p.text).join(" ");

    console.log(`mockGenerate returning assistant response with ${responseContent.length} parts.`); 
...
beforeEach(() => {
    ...
    mockLlmInstance = { generate: mockGenerate, send: mockSend, model: "mock-model", config: {} };
});
...
describe('E2E Multimodal Tests - Direct Agent Instantiation', () => {
    ...
    beforeEach(async () => { 
        mockConfig = {
            name: 'test-agent', instruction: 'You are a helpful test agent.', model: 'default-mock-model', agent_type: 'agent', servers: [],
        };
    ...
    it('should handle tool call and result', async () => {
        ...
        const response = await mockGenerate([assistantMessage, userResponseMessage]);
    ...
    it('should handle mixed text, image, and tool call/result flow', async () => {
        ...
        const assistantToolCall: PromptMessageMultipart = {
            role: Role.Assistant,
            content: [
                { 
                    type: 'tool_call', 
        ...
        const userToolResult: PromptMessageMultipart = {
            role: Role.User,
            content: [
                { 
                    type: 'tool_result', 
        ...
        const finalResponse = await mockGenerate([userPrompt, assistantToolCall, userToolResult]);
