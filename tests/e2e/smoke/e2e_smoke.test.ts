...
const textPromptLlm = { 
  send: jest.fn().mockImplementation((message: string) => {
    ...
    return Promise.resolve('Whiskers prowled the moonlit garden, tail twitching with anticipation. A rustling sound caught her attention. Mouse? Bird? She crouched low, ready to pounce. Her feline instincts took over as she leaped gracefully. The butterfly escaped, dancing just beyond reach. Whiskers watched, plotting her next move. Tomorrow, she thought. Tomorrow the butterfly would be hers.');
  }),
  generate: jest.fn().mockImplementation((messages: any[]) => {
    // Generate a 50-word story about cats
    return Promise.resolve({
      first_text: () => 'Whiskers prowled the moonlit garden, tail twitching with anticipation. A rustling sound caught her attention. Mouse? Bird? She crouched low, ready to pounce. Her feline instincts took over as she leaped gracefully. The butterfly escaped, dancing just beyond reach. Whiskers watched, plotting her next move. Tomorrow, she thought. Tomorrow the butterfly would be hers.',
      all_text: () => 'Whiskers prowled the moonlit garden, tail twitching with anticipation. A rustling sound caught her attention. Mouse? Bird? She crouched low, ready to pounce. Her feline instincts took over as she leaped gracefully. The butterfly escaped, dancing just beyond reach. Whiskers watched, plotting her next move. Tomorrow, she thought. Tomorrow the butterfly would be hers.'
    });
  })
};
...
const toolCallLlm = { 
  send: jest.fn().mockImplementation((message: string) => {
    if (message.toLowerCase().includes('weather')) {
      return Promise.resolve("I'll check the weather for you. It's sunny in London today!");
    } else if (message.toLowerCase().includes('shirt') || message.toLowerCase().includes('colour')) {
...
jest.mock('../../../src/core/directFactory', () => ({
  getModelFactory: jest.fn().mockImplementation((_context, modelName) => {
    ...
    return () => {
      ...
      if (['gpt-4.1', 'gpt-4.1-nano', 'gpt-4.1-mini', 'gpt-4o-mini', 'haiku35', 'deepseek', 'generic.llama3.2:latest', 'openrouter.google/gemini-2.0-flash-001', 'google.gemini-2.0-flash'].includes(modelName)) {
        // For text prompting tests
        return textPromptLlm;
      } else if (['gpt-4.1', 'gpt-4o'].includes(modelName)) {
        // For structured output tests
        return structuredLlm;
      } else if (['deepseek', 'haiku35', 'gpt-4o', 'gpt-4.1', 'gpt-4.1-nano', 'gpt-4.1-mini', 'google.gemini-2.0-flash', 'openrouter.google/gemini-2.0-flash-001', 'openrouter.anthropic/claude-3.7-sonnet'].includes(modelName)) {
...
const textPromptModels = ['gpt-4.1', 'gpt-4.1-mini', 'haiku35', 'generic.llama3.2:latest'];
const multiTextBlockModels = ['gpt-4o-mini', 'haiku35', 'openrouter.google/gemini-2.0-flash-001'];
  ...
  textPromptModels.forEach(modelName => {
    it(`should process basic textual prompting with ${modelName}`, async () => {
      ...
      fast.agent({ 
        ...
        const response = await agent.send(Prompt.user("write a 50 word story about cats"));
  ...
  multiTextBlockModels.forEach(modelName => {
    it(`should process multiple text blocks with ${modelName}`, async () => {
      ...
      fast.agent({ 
        ...
        const response1 = await agent.default.generate(
        ...
        const response2 = await agent.default.generate(
          [
  ...
  structuredModels.forEach(modelName => {
    it(`should generate structured weather forecast data with ${modelName}`, async () => {
      ...
      fast.agent({ 
        ...
        instruction: 'You are a helpful assistant that provides synthesized weather data for testing purposes.',
        model: modelName
      }, async (agent: BaseAgent) => {
  ...
  toolCallModels.forEach(modelName => {
    it(`should make basic tool calls with ${modelName}`, async () => {
      ...
      fast.agent({ 
        ...
        // Test tool call
        const response = await agent.send(Prompt.user("what is the weather in london"));
  ...
  noArgsToolModels.forEach(modelName => {
    it(`should make tool calls with no arguments with ${modelName}`, async () => {
      ...
      fast.agent({ 
        ...
        const response = await agent.send(Prompt.user("get the shirt colour"));
