...
// Define a mock LLM object
const mockLlm = {
  generate: jest.fn(),
  send: jest.fn(),
  // Add other methods if needed by the Agent class
};
...
describe('Agent Class', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    agent = new Agent(mockConfig, [], true, mockSocket as any, undefined);
    // Attach the mock LLM to satisfy the Agent's requirement
    (agent as any)._llm = mockLlm;
  });
