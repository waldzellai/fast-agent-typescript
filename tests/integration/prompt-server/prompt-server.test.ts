...
describe('FastAgent Prompt Server Integration Tests', () => {
  ...
  const mockGetPrompt = jest
    .fn()
    .mockImplementation(
      (promptName: string, args?: any, serverName?: string) => {
        if (promptName === 'simple') {
          return Promise.resolve({
            messages: [
              {
                role: 'user',
                content: [{ text: 'simple, no delimiters', type: 'text' }],
              },
            ],
          });
            ...
            messages: [
              {
                role: 'user',
                content: [
                  { text: `this is ${product} by ${company}`, type: 'text' },
                ],
              },
            ],
          });
          ...
          return Promise.resolve({
            messages: [
              {
                role: 'user',
                content: [{ text: 'good morning', type: 'text' }],
              },
              {
                role: 'assistant',
                content: [{ text: 'how may i help you?', type: 'text' }],
              },
            ],
          });
            ...
            messages: [
              {
                role: 'user',
                content: [
                  { text: `hello, my name is ${userName}`, type: 'text' },
                ],
              },
              {
                role: 'assistant',
                content: [
                  {
                    text: `nice to meet you. i am ${assistantName}`,
                    type: 'text',
                  },
                ],
              },
          ...
          return Promise.resolve({
            messages: [
              {
                role: 'user',
                content: [{ text: 'text message 1', type: 'text' }],
              },
              {
                role: 'assistant',
                content: [{ text: 'response 1', type: 'text' }],
              },
              {
                role: 'user',
                content: [{ text: 'text message 2', type: 'text' }],
              },
              {
                role: 'user',
                content: [
              ...
              {
                role: 'assistant',
                content: [{ text: 'I see an image', type: 'text' }],
              },
            ],
          });
        }
  ...
  const mockListPrompts = jest.fn().mockImplementation(() => {
    return Promise.resolve({
      prompts: [
        ...
        {
          name: 'multi_sub',
          description: 'Multi-turn conversation with substitutions',
        },
  ...
  it('should handle prompt with no delimiters', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        // result.messages should already be in a usable format
        const messages = result.messages as PromptMessageMultipart[]; // Assert type for clarity
  ...
  it('should handle prompt with no delimiters and variables', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        const result = await (agent as any).test.get_prompt('simple_sub', {
          product: 'fast-agent',
          company: 'llmindset',
        });

        // result.messages should already be in a usable format
        const messages = result.messages as PromptMessageMultipart[]; // Assert type

        // Verify prompt content
        expect(messages[0].content[0].type).toBe('text');
        expect((messages[0].content[0] as TextContent).text).toBe(
          'this is fast-agent by llmindset'
        ); // Access text content
        expect(messages[0].role).toBe('user');
        expect(messages.length).toBe(1);

        // Verify mock was called correctly
        expect(mockGetPrompt).toHaveBeenCalledWith('simple_sub', {
          product: 'fast-agent',
          company: 'llmindset',
        });
      }
  ...
  it('should handle multi-turn prompts', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        expect((messages[1].content[0] as TextContent).text).toBe(
          'how may i help you?'
        );
  ...
  it('should handle multi-turn prompts with substitution', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        const result = await (agent as any).test.get_prompt('multi_sub', {
          user_name: 'evalstate',
          assistant_name: 'HAL9000',
        });
        ...
        expect(messages[0].content[0].type).toBe('text');
        expect((messages[0].content[0] as TextContent).text).toBe(
          'hello, my name is evalstate'
        );
        expect(messages[0].role).toBe('user');
        expect(messages[1].content[0].type).toBe('text');
        expect((messages[1].content[0] as TextContent).text).toBe(
          'nice to meet you. i am HAL9000'
        );
        expect(messages[1].role).toBe('assistant');
        expect(messages.length).toBe(2);

        // Verify mock was called correctly
        expect(mockGetPrompt).toHaveBeenCalledWith('multi_sub', {
          user_name: 'evalstate',
          assistant_name: 'HAL9000',
        });
      }
  ...
  it('should get prompt with explicit server parameter', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        const prompt = await (agent as any).test.get_prompt(
          'simple',
          null,
          'prompts'
        );
        ...
        // Verify prompt content
        expect(getText(prompt.messages[0].content)).toBe(
          'simple, no delimiters'
        );
  ...
  it('should apply prompt with server parameter', async () => {
    ...
    fast.agent(
      ...
      async (agent: BaseAgent) => {
        ...
        const response1 = await (agent as any).test.apply_prompt(
          'simple',
          null,
          'prompts'
        );
        ...
        expect(
          response2.includes('test-product') ||
            response2.includes('test-company')
        ).toBe(true);
