...
app.agent(
  {
    name: 'programming',
    instruction:
      'You are a programming expert that helps with coding questions and provides code examples.',
  },
  async (agent) => {
...
app.agent(
  {
    name: 'history',
    instruction:
      'You are a history expert that provides detailed information about historical events and figures.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
...
app.agent(
  {
    name: 'science',
    instruction:
      'You are a science expert that explains scientific concepts and discoveries.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
...
app.router(
  {
    ...
    instruction:
      'A router that directs queries to the appropriate specialized agent.',
  ...
  async (router) => {
    ...
    for (const query of queries) {
      console.log(`\nRouting query: "${query}"`);
      const response = await (router as any).execute(query);
    ...
    await router.prompt(
      'Ask me anything about programming, history, or science!'
    );
  }
);
