...
app.agent({
  name: 'formatter',
  instruction: 'You are a formatting assistant that formats text into a professional presentation.',
}, async (agent) => {
  // This function is called when the agent is run directly
  console.log('Formatter agent started directly.');
  const response = await agent.send('Format the following bullet points into a professional presentation:\n- Static typing\n- Better tooling\n- Object-oriented features');
...
app.chain({
  name: 'research-chain',
  instruction: 'A chain that researches, summarizes, and formats information.',
  ...
  const response = await chain.execute('What are the key benefits of TypeScript over JavaScript?');
