...
app.agent(
  {
    name: 'outliner',
    instruction:
      'You are an assistant that creates structured outlines for content.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
    console.log('Outliner agent started directly.');
    const response = await agent.send(
      'Create an outline for an article about artificial intelligence.'
    );
...
app.agent(
  {
    name: 'writer',
    instruction:
      'You are a content writer that creates well-written articles based on outlines and research.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
    console.log('Writer agent started directly.');
    const response = await agent.send(
      'Write a short article based on this outline: I. Introduction to AI II. History of AI III. Modern Applications IV. Future Directions'
    );
...
app.agent(
  {
    name: 'editor',
    instruction:
      'You are an editor that improves and polishes written content for clarity, grammar, and style.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
    console.log('Editor agent started directly.');
    const response = await agent.send(
      'Edit and improve this article for clarity and style: [article text]'
    );
...
app.orchestrator(
  {
    ...
    instruction:
      'An orchestrator that coordinates the process of researching, outlining, writing, and editing content.',
  ...
  async (orchestrator) => {
    ...
    for (const topic of topics) {
      ...
      const response = await (orchestrator as any).execute(
        `Create a comprehensive, well-researched article about "${topic}". The article should be informative, engaging, and well-structured.`
      );
      console.log('\nFinal Content:');
