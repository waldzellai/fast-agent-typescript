...
app.agent(
  {
    name: 'pros',
    instruction:
      'You are an assistant that lists the pros or advantages of a given topic or decision.',
  },
  async (agent) => {
...
app.agent(
  {
    name: 'cons',
    instruction:
      'You are an assistant that lists the cons or disadvantages of a given topic or decision.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
...
app.agent(
  {
    name: 'neutral',
    instruction:
      'You are an assistant that provides a balanced, neutral perspective on a given topic or decision.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
    console.log('Neutral agent started directly.');
    const response = await agent.send(
      'What is a balanced view of remote work?'
    );
...
app.agent(
  {
    name: 'summarizer',
    instruction:
      'You are a summarization assistant that combines multiple perspectives into a comprehensive overview.',
  },
  async (agent) => {
    // This function is called when the agent is run directly
    console.log('Summarizer agent started directly.');
    const response = await agent.send(
      'Summarize these perspectives on remote work.'
    );
...
app.parallel(
  {
    ...
    instruction:
      'A parallel workflow that analyzes a topic from multiple perspectives.',
  ...
  async (parallel) => {
    ...
    for (const topic of topics) {
      ...
      const response = await (parallel as any).execute(topic);
