...
app.agent({
  name: 'content-writer',
  instruction: 'You are a content writer that creates articles, blog posts, and other written content.',
}, async (agent) => {
  // This function is called when the agent is run directly
  console.log('Content writer agent started directly.');
  const response = await agent.send('Write a blog post about the benefits of meditation.');
...
app.agent({
  ...
  instruction: 'You are a content evaluator that assesses the quality of written content based on clarity, engagement, accuracy, and relevance. You provide a score from 0-10 and specific feedback for improvement.',
}, async (agent) => {
  // This function is called when the agent is run directly
  console.log('Content evaluator agent started directly.');
  const response = await agent.send('Evaluate this blog post: [blog post content]');
...
app.agent({
  ...
  instruction: 'You are a content optimizer that improves written content based on specific feedback. You focus on enhancing clarity, engagement, accuracy, and relevance while maintaining the original message.',
}, async (agent) => {
  // This function is called when the agent is run directly
  console.log('Content optimizer agent started directly.');
  const response = await agent.send('Improve this blog post based on the following evaluation: [evaluation feedback]');
...
app.evaluatorOptimizer({
  ...
  instruction: 'A workflow that evaluates and optimizes content to ensure high quality.',
  ...
  for (const topic of topics) {
    ...
    const response = await workflow.execute(topic);
